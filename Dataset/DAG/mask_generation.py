...
from utils import DataLoader3D
    ...
    def __call__(self, **data_dict):
        ...
        for n in range(bs):
            if self.not_in_background:
                ...
                print(indices_nonzero.shape, margin.shape, image.shape, (np.array(image_size)[None]).shape)
...
class resize_operation():
    def __init__(self, patch_size = (128,128,128), mode = "bilinear"):
        self.patch_size = patch_size
        self.mode = mode
        import torchvision
        self.resize = torchvision.transforms.Resize(self.patch_size, interpolation=torchvision.transforms.InterpolationMode.BICUBIC)
    ...
    def __call__(self, **data_dict):
        ...
        import torch
        for i in range(bs):
            data = self.resize(torch.from_numpy(data_dict["data"][0][i]).unsqueeze(0))
            data_dict["data"][0][i] = data.squeeze(0).numpy()
        return data_dict
